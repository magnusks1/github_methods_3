...
```{r}
pacman::p_load(tidyverse, readbulk, dplyr, lme4)
...
m.subj.sl <- glmer(correct ~ target.frames + (1 + subject|subject), family = 'binomial', data = df)
logLik(m.subj.sl)
```

**We are unsure if we should keep the predicted by 1 or not.** Use LogLik to test the models.
