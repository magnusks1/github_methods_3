...
pacman::p_load('lme4', 'dplyr', 'ggplot2', 'stats', 'MuMIn')
```
...
# Providing an overview of the data set and their data type
glimpse(politeness)

# Encoding columns 'subject', 'gender' and 'attitude' as factors
...
new_politeness <- politeness %>% 
  filter(subject == 'F1')

# Linear model 1
model_integer <- lm(f0mn ~ scenario, data = new_politeness)
model_integer

# Encoding scenario as factor
scenario_asfactor <- new_politeness %>% 
  mutate(
    scenario = as.factor(scenario))

# Linear model 2
model_factor <- lm(f0mn ~ scenario, data = scenario_asfactor)
model_factor

# i.
model.matrix(model_integer)
model.matrix(model_factor)
# The integer encoding interprets the scenarios as one whole, whilst the factor encoding divides these into 7 scenarios. This creates a better overview.

# ii.
# The factor encoding is definitely best ... but why?
...
ggplot(data = politeness_factor, aes(x = scenario, y = f0mn, color = attitude)) +
  geom_point() +
  facet_wrap(~ subject)+
  theme_bw()
...
```{r}
#i
modeli <- lm(f0mn ~ gender, data = politeness)
modeli

# ii
modelii <- lmer(f0mn ~ gender + (1 | scenario), data = politeness)
modelii

# iii
modeliii <- lmer(f0mn ~ gender + (1 | subject), data = politeness)
modeliii

#iv
modeliv <- lmer(f0mn ~ gender + (1 | subject) + (1 | scenario), data = politeness)
modeliv

#v
# Model 4 has the lowest residual standard deviation with a value of 30.658.
AIC(modeli, modelii, modeliii, modeliv)
# Model 4 also has the lowest AIC with a value of 2092.482

#vi
r.squaredGLMM(modelii)
r.squaredGLMM(modeliii)
r.squaredGLMM(modeliv)
# Model 4 explains the most variance (R2c = 0.8078), which makes sense as it takes the biggest part of the data into account.
```
...
new_df <- politeness %>%
  ...
  summarise(f0mn1=mean(f0mn))
  
glimpse(new_df)

# ii
newmodeli <- lm(f0mn1 ~ gender, data = new_df)
newmodeli

# iii
qqnorm(residuals(newmodeli))
qqline(residuals(newmodeli))

qqnorm(residuals(modeli))
qqline(residuals(modeli))

#iv
qqnorm(residuals(modeliv))
qqline(residuals(modeliv))
# It looks fine.
```
