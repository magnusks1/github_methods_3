...
# Could also be individual models per subject .. depends on how you read the question.

model.2.1.1 <- lmer(rt.obj ~ 1 + (1 | subject) , data = df_4participants)
model.2.1.1
# should probably be 'lmer(rt.obj ~ 1)' only
...
#It does not improve it, but it looks different. It might actually improve the data a bit as it is not as skewed as before — it now looks more normally distributed but still not perfect.
```
